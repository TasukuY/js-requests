...
sayHelloButton.addEventListener('mouseleave', mouseLeave);
...
const repeatMyParam = () => {
    ...
    axios.get(`${baseURL}/repeat/testing`)
        .then(res => {
...
function queryTest(){
    ...
    axios.get(`${baseURL}/query-test/?name=test`)//?name=Israel Getu & name=test
        .then(res => {
...
const ohMy = () => {
    // YOUR CODE HERE
    //sending a request to the sever to give us 'http://localhost:3000/animals'. 
    axios.get(`${baseURL}/animals`)
        //if we can get the 'http://localhost:3000/animals' from the server as a response,
        .then(res => {
            //do this.
            //console.log(res.data);
            for(let i = 0; i < res.data.length; i++){
                ...
                bodyTag.appendChild(newPelement);
            }

        })
        //if we cannot get the 'http://localhost:3000/animals' from the server as a response, console.log error.
        .catch(err => console.log(err))
}
...
/*
    ...
    get a message => 'You sent an empty query!'
    2: Send more than 1 query on the URL -- what happened? 
    get a message => message 'You sent more than 1 query!'
*/
